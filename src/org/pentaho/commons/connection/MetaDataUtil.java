...
public class MetaDataUtil {
   ...
   * row header names (optional) and a format string for the column headers.
   * The number of column names returned will be equal to the number of row 
   * headers in the first row of row headers and the number of columns in the
   * first row of column headers. It is assumed that the row and column headers
   * arrays are not ragged. 
   * The row headers can be null.
   * The format string can be null.
   * @param columnHeaders
   * @param rowHeaders
  ...
  public static String[] generateColumnNames( Object columnHeaders[][], Object rowHeaders[][], String rowHeaderNames[], String columnNameFormatStr ) {
    ...
    if( rowHeaderCount>0 ) {
      if( haveRowHeaderNames ) {
        ...
        for( int idx=0; idx<rowHeaderCount; idx++ ) {
          columnNames[idx] = Messages.getString("MetaDataUtil.USER_DEFAULT_ROW_HEADER_NAME", Integer.toString(idx+1)); //$NON-NLS-1$
        }
      }
    }
